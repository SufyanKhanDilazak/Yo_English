...
const ScrollExpand: React.FC<ScrollExpandProps> = ({
  ...
  useEffect(() => {
    ...
    const motionQuery = supportsMatchMedia
    ...
    const measure = () => {
      const c = propsRef.current;
      const nextStageH = c.useWindowScroll ? window.innerHeight : root.clientHeight;
      if (nextStageH <= 0) return;
      const rootWidth = root.clientWidth || nextStageH;

      stageH = nextStageH;
      stage.style.height = `${stageH}px`;
      track.style.height = `${stageH * (1 + Math.max(0, c.scrollDistance) + Math.max(0, c.holdDistance))}px`;
      stage.style.setProperty('--se-title-size', `${clamp(rootWidth * 0.075, 20, 84)}px`);
    };
    ...
    // Only run scroll math while this component is near the viewport —
    // avoids doing work on every page scroll when it's off-screen.
    const io =
  ...
  return (
    <section
      ...
      className={`relative w-full h-full ${
        useWindowScroll
          ? ''
          : 'overflow-y-auto overflow-x-hidden overscroll-contain [touch-action:pan-y] [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden'
      } ${className}`.trim()}
      style={style}
      {...rest}
    >
      <div ref={trackRef} className="relative w-full">
        <div
          ref={stageRef}
          className="sticky top-0 w-full overflow-hidden [contain:layout_paint_style] [--se-title-size:4rem]"
        >
          <div
            ref={frameRef}
            className="absolute inset-0 [clip-path:inset(21%_29%_21%_29%_round_24px)] [contain:layout_paint] [will-change:clip-path]"
          >
            {media}
            <div
              ref={scrimRef}
